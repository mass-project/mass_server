...
class SampleTestCase(FlaskTestCase):
    ...
    def _create_file_sample(self):
        ...
        file_sample = Sample.create_or_update(unique_features=unique_features, first_seen=self._timestamp)
        return file_sample
    ...
    def test_sample_without_unique_features(self):
        kwargs = {}
    ...
    def test_is_file_name_correct(self):
        ...
        self.assertIn('gpl-2.0.txt', file_sample.unique_features.file.file_names)

    def test_is_file_size_correct(self):
        file_sample = self._create_file_sample()
        self.assertEqual(file_sample.unique_features.file.file_size, 18092)

    def test_is_md5sum_correct(self):
        file_sample = self._create_file_sample()
        self.assertEqual(file_sample.unique_features.file.md5sum, 'b234ee4d69f5fce4486a80fdaf4a4263')

    def test_is_sha1sum_correct(self):
        file_sample = self._create_file_sample()
        self.assertEqual(file_sample.unique_features.file.sha1sum, '4cc77b90af91e615a64ae04893fdffa7939db84c')

    def test_is_sha256sum_correct(self):
        file_sample = self._create_file_sample()
        self.assertEqual(file_sample.unique_features.file.sha256sum, '8177f97513213526df2cf6184d8ff986c675afb514d4e68a404010521b880643')

    def test_is_sha512sum_correct(self):
        file_sample = self._create_file_sample()
        self.assertEqual(file_sample.unique_features.file.sha512sum, 'aee80b1f9f7f4a8a00dcf6e6ce6c41988dcaedc4de19d9d04460cbfb05d99829ffe8f9d038468eabbfba4d65b38e8dbef5ecf5eb8a1b891d9839cda6c48ee957')

    def test_is_ssdeep_hash_correct(self):
        file_sample = self._create_file_sample()
        self.assertEqual(file_sample.unique_features.file.ssdeep_hash, '384:ghUwi5rpL676yV12rPd34ZomzM2FR+dWF7jUI:gmFWixMFzMdm7jUI')

    def test_is_shannon_entropy_correct(self):
        file_sample = self._create_file_sample()
        self.assertAlmostEqual(file_sample.unique_features.file.shannon_entropy, 4.666564742606159)
