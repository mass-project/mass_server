...
def sample_detail(sample_id):
    try:
        ...
        return render_template('sample_detail.html', sample=sample, reports=reports, activity=sorted_activity,
                               ...
                               scheduled_analyses=scheduled_analyses, request_form=request_form)
    except DoesNotExist:
...
def _process_comment_form(form, sample):
    if form.validate_on_submit():
        sample.add_comment(form.data['comment'], TimeFunctions.get_timestamp(),
...
def _process_request_form(form, sample):
    if form.validate_on_submit() and current_authenticated_entity.is_authenticated:
        priority = form.data['priority']
        analysis_system = AnalysisSystem.objects.get(identifier_name=form.data['analysis_system'])
        try:
